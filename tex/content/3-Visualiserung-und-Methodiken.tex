...
Durch diese Übersicht können schnell Entscheidungen getroffen werden, welche Branches in den Fokus der Entwicklung gerückt werden müssen. Alte oder bereits in den Codestand überführte Branches können gelöscht werden. Branches mit schlechten Kennzahlen können zu einem \gls{Refactoring} eingeplant werden.
...
\begin{figure}[htbp]
  \includegraphics[width=\textwidth, height=\textheight, keepaspectratio]
    {resources/bar-chart.pdf}
  \caption{Balkendiagramm zur Komplexität der Individualsoftware - Eine Klasse sticht hervor}
  \label{bar-chart}
\end{figure}
...
Zudem unterstützen die nachfolgenden Methodiken das Verfahren \glqq \nameref{temporary-releases}\grqq{} aus Kapitel~\ref{temporary-releases}. Das Verfahren ist abhängig von \gls{Code-Kopplung} und profitiert von einer besseren Testqualität.
...
\section[Umfrage zur Nutzung von VCS und Tests]{Umfrage zum Nutzungsverhalten von Versionsverwaltungssystemen und Tests}
...
Test werden vom Großteil der Teilnehmer verwendet. Insbesondere Unit- (72\%), Funktions\hyp{} und Integrationstests (68\%) wurden mit einer hohen Nutzung angegeben. Sichtbar weniger wurden Oberflächentests mit 48\% verwendet.
...
\item Wie verhalten sich die Metriken der Feature-Branches untereinander und zum Haupt\hyp{}Branch?
\end{itemize}
...
Zur weiteren Bewertung der Branches müssen Merge\hyp{}Konflikte zwischen den Branches ermittelt werden. Branches, die einen Konflikt aufweisen, sollten in der Release-Übersicht markiert und gruppiert werden.
...
Das betrachtete Problem betrifft sowohl Repository-Artefakte, als auch Projekt- und Release\hyp{}Details. Weiter müssen Metriken und Testergebnisse protokolliert werden. Da die Erstellung von Metriken und Tests eine größere Zeitspanne benötigen kann, sollten die Ergebnisse gesichert werden. 
...
Um ein Projekt zu registrieren wird der Pfad zu einer \glqq composer.json\grqq{}\hyp{}Datei angegeben. Die Datei beschreibt den Namen, das zugehörige Repository und die Abhängigkeiten des Projektes.
