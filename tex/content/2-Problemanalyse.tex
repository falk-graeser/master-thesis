...
Gitflow\footcite{nvie-git-branch-model} ist ein weiterer bekannter Workflow mit Git. Im Gegensatz zum simplen GitHub-Workflow liegt beim Gitflow der Schwerpunkt auf der Erstellung eines Releases, also einer Menge an Features. 

Gitflow unterscheidet wie viele andere Git-Arbeitsmodelle zwischen langlebigen und kurzlebigen Branches. Die langlebigen Branches sind hierbei ``master'' und ``development''. Der Master darfhierbei ausschließlich Release-Commits enthalten, also Commits, welche mit einem Tag referenziert werden.
