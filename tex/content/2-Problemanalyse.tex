...
zentraler Punkt auf dem alle Versionsinformationen gespeichert werden. Dies ermöglicht einen einfachen Kontrollfluss und führt nur zu geringfügigen Abweichungen unter allen beteiligten Entwicklern. Damit einhergehend ist eine Übersicht aller Änderungen problemlos möglich. 
...
In Abbildung \ref{dependency-mgmg-composer-wikia} ist ein Ausschnitt einer composer.json und der zugehörigen Lock-Datei zu sehen. Beide Dateien sind im JSON-Format und nutzen jeweils eine DSL für die Paketverwaltung Composer\footcite{composer-json}. In der Abbildung wird die Verwendung von regulären Ausdrücken in Verbindung mit einem Paketnamen dargestellt. Zudem wird die Präzisierung der Paketdatei zur Lock-Datei deutlich.
